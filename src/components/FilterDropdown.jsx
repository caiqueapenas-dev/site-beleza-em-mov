          ...
          <div className="p-4 space-y-6">
            ...
            <div>
              ...
              <ul className="space-y-1">
                {categories.map(cat => (
                  <li key={cat}><button onClick={() => handleSelect(onCategoryChange, cat)} className={`capitalize text-gray-600 ${activeCategory === cat ? 'font-bold text-cyan-700' : ''}`}>{cat === 'todos' ? 'Mostrar Todos' : cat}</button></li>
                ))}
              </ul>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {sizes.map(size => (
                  <button key={size} onClick={() => handleSelect(onSizeChange, size)} className={`border rounded-md px-3 py-1 text-xs transition-colors ${selectedSize === size ? 'bg-gray-900 text-white border-gray-900' : 'hover:bg-gray-200'}`}>{size}</button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {availableColors.map(color => (
                  <button key={color.nome} onClick={() => handleSelect(onColorChange, color.nome)} title={color.nome} className={`w-6 h-6 rounded-full border transition-transform hover:scale-110 ${selectedColor === color.nome ? 'ring-2 ring-offset-2 ring-cyan-500' : ''}`} style={{ backgroundColor: color.hex }} />
                ))}
              </div>
            </div>
            
            <button onClick={() => { onClearFilters(); setIsOpen(false); }} className="w-full text-center text-sm text-cyan-600 hover:underline pt-2 border-t">
              Limpar Filtros
            </button>
          </div>
        </div>
